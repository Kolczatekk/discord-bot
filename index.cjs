...
function getRateForPlnAmount(pln, serverRaw) {
  ...
  if (server === "MINESTAR_LF") {
    if (Number(pln) >= 50) return 400;
    return 300;
  }
    ...
    try {
      ...
      const trybSelect = new StringSelectMenuBuilder()
        ...
        .addOptions(
          ...
          { label: "MineStar LF", value: "MINESTAR_LF" }
        );
  ...
  try {
    ...
    if (typ === "otrzymam") {
      ...
      if (server === "ANARCHIA_BOXPVP") {
        ...
        const estimatedPln300 = waluta / 300;
        rate = estimatedPln300 >= 50 ? 400 : 300;
      } else {
        // fallback
        const estimatedPln4500 = waluta / 4500;
        rate = estimatedPln4500 >= 100 ? 5000 : 4500;
...
async function showZakupModal(interaction) {
  ...
  const serwerInput = new TextInputBuilder()
    ...
    .setPlaceholder("Tryb LifeSteal na MineStar")
    .setStyle(TextInputStyle.Short)
    .setRequired(true);
...
async function handleModalSubmit(interaction) {
  ...
  try {
    ...
    if (parentToUse && !forceOwnerOnlyVisibility) {
      ...
      const isPurchaseTicket = ticketType && (ticketType.startsWith("zakup-") || ticketType === "zakup" || ticketTypeLabel === "ZAKUP" || ticketTypeLabel === "ZAKUP AUTO RYNKU");
...
// Helper: Sprawdza i przyznaje nagrody za zaproszenia
async function checkAndGiveInviteRewards(guild, userId, currentCount) {
  const guildId = guild.id;
  ...
  if (!rewardLevelsMap.has(userId)) {
    rewardLevelsMap.set(userId, new Set());
  }
  const userRewardLevels = rewardLevelsMap.get(userId);

  // Progi nagród
  const tiers = [
    { level: 5, rewardAmount: 90000, rewardText: "90k$" },
    { level: 10, rewardAmount: 200000, rewardText: "200k$" }
  ];

  const generatedCodes = [];

  for (const tier of tiers) {
    const levelStr = tier.level.toString();
    if (currentCount >= tier.level && !userRewardLevels.has(levelStr)) {
      const rewardCode = generateCode();
      const CODE_EXPIRES_MS = 24 * 60 * 60 * 1000; // 24 godziny
      ...
      const codeData = {
        ...
        reward: tier.rewardText, // Kompatybilność
        type: "invite_cash",
        discount: 0,
        used: false,
        expiresAt,
      ...
      activeCodes.set(rewardCode, codeData);
      ...
      try {
        ...
        console.error(`[rewards] Błąd zapisu kodu do Supabase dla ${userId}:`, err);
      }

      // Oznacz ten próg jako odebrany
      userRewardLevels.add(levelStr);
      generatedCodes.push({ code: rewardCode, tier });
      console.log(`[rewards] Użytkownik ${userId} otrzymał nagrodę za próg ${tier.level} zaproszeń`);
    }
  }
  ...
  if (generatedCodes.length > 0) {
    ...
    const rewardsGivenMap = inviteRewardsGiven.get(guildId);
    const alreadyGiven = rewardsGivenMap.get(userId) || 0;
    rewardsGivenMap.set(userId, alreadyGiven + generatedCodes.length);
    ...
    try {
      ...
      const expiresAtSeconds = Math.floor((Date.now() + 24 * 60 * 60 * 1000) / 1000);
      ...
      for (const item of generatedCodes) {
        const dmEmbed = new EmbedBuilder()
          ...
          .setDescription(
            ...
            "```\n" +
            `\`👤\` × **Użytkownik:** <@${userId}>\n` +
            `\`🎉\` × **Gratulacje! Otrzymałeś nagrodę za zaproszenia (próg ${item.tier.level})!**\n` +
            `\`💸\` × **Kod nagrody:**\n` +
            "```\n" +
            item.code +
            "\n```\n" +
            `\`💰\` × **Wartość:** \`${item.tier.rewardText}\`\n` +
        ...
        await u.send({ embeds: [dmEmbed] }).catch(() => {
          console.error(`Nie udało się wysłać DM z nagrodą do użytkownika ${userId}`);
        });
      }
    } catch (e) {
...
client.on(Events.GuildMemberAdd, async (member) => {
  try {
    ...
    if (inviterId) {
      ...
      await checkAndGiveInviteRewards(member.guild, inviterId, currentCount);
    }
...
async function handleSprawdzZaproszeniaCommand(interaction) {
  ...
  let rewardMessage = "";
  if (displayedInvites < 5) {
    const missing = 5 - displayedInvites;
    rewardMessage = `> \`💸\` × **Brakuje ci zaproszeń do nagrody 90k$ (próg 5):** \`${missing}\``;
  } else if (displayedInvites < 10) {
    const missing = 10 - displayedInvites;
    rewardMessage = `> \`💸\` × **Brakuje ci zaproszeń do nagrody 200k$ (próg 10):** \`${missing}\``;
  } else {
    rewardMessage = `> \`💸\` × **Osiągnąłeś już wszystkie nagrody za zaproszenia!**`;
  }
...
async function handleZaprosieniaStatsCommand(interaction) {
  ...
  if (category === "prawdziwe") {
    await checkAndGiveInviteRewards(interaction.guild, user.id, newVal);
  }
